...
class NaiveBayes:
  ...
  def classify(self, words):
    ...
    """
    prob = {'pos': 0.0, 'neg': 0.0}
    for klass in 'pos', 'neg':
    	# calculate the prior probability
    	prior = 1.0 * self.doc_count_byklass[klass] / self.totaldoc_count
    	# prepare the denominator for the following likelihood
  ...
  def addExample(self, klass, words):
    ...
    """
    # prepare for prior
    self.totaldoc_count += 1
    self.doc_count_byklass[klass] += 1
  ...
  def filterStopWords(self, words):
    ...
    * Filters stop words found in self.stopList.
    ...
    words_filter = []
    for word in words:
    	if  word in self.stopList and word.strip() != '':
